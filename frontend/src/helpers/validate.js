...
export function validateForm(
  ...
  if (registration_code !== null) {
    if (registration_code.length != 10) {
      form_catch.registrationCodeInvalidLength =
        "*Registration code needs to be 10 characters";
    } else if (!REGCODE_REGEX.test(registration_code)) {
      form_catch.registrationCodeInvalidLength =
        '*Allowed characters are only "a" to "f" and "0" to "9"';
    }
  }
